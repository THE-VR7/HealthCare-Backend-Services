...
@Transactional
public class HealthCareBackendApplicationTests {
	public String token = "";
	...
	public void test2() throws Exception {
		...
		byte[] iJson = toJson(u);
		MvcResult result = mockMvc.perform(post("/register").content(iJson).contentType(MediaType.APPLICATION_JSON)
	...
	public void test3() throws Exception {
		...
		mockMvc.perform(get("/viewprofile/user1").header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
	...
	public void test4() throws Exception {
		...
		MvcResult result = mockMvc.perform(post("/signin").content(iJson1).contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andReturn();
		token = JsonPath.read(result.getResponse().getContentAsString(), "$.token");

		registerPatientents(token);
		MvcResult result1 = mockMvc
				.perform(get("/patients/list").header("Authorization", "Bearer " + token)
		...
		mockMvc.perform(get("/patients/view/" + patientid).header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
				.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				.andExpect(MockMvcResultMatchers.jsonPath("$.patient_name").value("patient1"));
		mockMvc.perform(get("/patients/view/" + patientid1).header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
				.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
	...
	private void registerPatientents(String token1) throws Exception {
		...
		byte[] iJson = toJson(p);
		mockMvc.perform(post("/patients/register").header("Authorization", "Bearer " + token1).content(iJson)
				.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))

				.andExpect(status().isOk());
		Patient p1 = new Patient("patient2", "patient2@hcs.com", "8989998989", new Date());
		byte[] iJson2 = toJson(p1);
		mockMvc.perform(post("/patients/register").header("Authorization", "Bearer " + token1).content(iJson2)
				.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))
	...
	public void test5() throws Exception {
		...
		mockMvc.perform(post("/appointment/register").header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
				.content(iJson2).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	}
	...
	public void test6() throws Exception {
		...
		byte[] iJson2 = toJson(A1);
		MvcResult result1 = mockMvc
				.perform(post("/appointment/register").header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
						.content(iJson2).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andReturn();
